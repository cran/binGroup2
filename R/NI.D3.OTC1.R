...
NI.D3.OTC1 <- function(p, Se, Sp, group.sz, obj.fn, weights = NULL, 
  ...
  for (I in set.of.I) {
    ...
    # do not include the first column, which would test the initial 
    # group twice
    possible.groups <- as.matrix(parts(n = I)[,-1])
    
    save.it <- matrix(data = NA, nrow = ncol(possible.groups), 
                      ncol = max(set.of.I) + 15)
    ...
    for (c in 1:ncol(possible.groups)) {
      ...
      if (is.null(dim(weights))) {
        ...
        if (dim(weights)[1] >= 6) {
          GR6 <- GR.func(ET = ET, p.vec = p.vec, PSe.vec = 
                           PSe.vec, PSp.vec = PSp.vec, 
                         D1 = weights[6,1], D2 = weights[6,2])
        } else {GR6 <- NA}
      }
      
      save.it[counter,] <- c(p[1], I, ET, ET / I, MAR, GR1 / I, GR2 / I, 
    ...
    if (I == set.of.I[1]) {
      if (obj.fn[1] == "ET") {
        top.configs <- as.matrix(save.it[order(save.it[,4]),])[1:min(nrow(save.it), num.top), 
                                                               c(1:3,4,12:ncol(save.it))]
      } else if (obj.fn[1] == "MAR") {
        top.configs <- as.matrix(save.it[order(save.it[,5]),])[1:min(nrow(save.it), num.top), 
                                                               c(1:3,5,12:ncol(save.it))]
      } else if (obj.fn[1] == "GR") {
        top.configs <- as.matrix(save.it[order(save.it[,6]),])[1:min(nrow(save.it), num.top), 
                                                               c(1:3,6,12:ncol(save.it))]
      }
    } else if (I > set.of.I[1]) {
      if (obj.fn[1] == "ET") {
        top.configs <- rbind(top.configs, as.matrix(save.it[order(save.it[,4]),])[1:min(nrow(save.it), num.top), 
                                                                                  c(1:3,4,12:ncol(save.it))])
      } else if (obj.fn[1] == "MAR") {
        top.configs <- rbind(top.configs, as.matrix(save.it[order(save.it[,5]),])[1:min(nrow(save.it), num.top), 
                                                                                  c(1:3,5,12:ncol(save.it))])
      } else if (obj.fn[1] == "GR") {
        top.configs <- rbind(top.configs, as.matrix(save.it[order(save.it[,6]),])[1:min(nrow(save.it), num.top), 
                                                                                  c(1:3,6,12:ncol(save.it))])
      }
    }
    
    # find the best configuration for each initial group size I, out of all possible configurations
    save.ET[count,] <- save.it[save.it[,4] == min(save.it[,4]), 
                               c(1:3,4,12:ncol(save.it))]
    if (class(try(save.MAR[count,] <- save.it[save.it[,5] == min(save.it[,5]), 
                                              c(1:3,5,12:ncol(save.it))],silent = T)) != "try-error") {
      save.MAR[count,] <- save.it[save.it[,5] == min(save.it[,5]), 
                                  c(1:3,5,12:ncol(save.it))]
    }
    if (class(try(save.GR1[count,] <- save.it[save.it[,6] == min(save.it[,6]), 
                                              c(1:3,6,12:ncol(save.it))],silent = T)) != "try-error") {
      save.GR1[count,] <- save.it[save.it[,6] == min(save.it[,6]), 
                                  c(1:3,6,12:ncol(save.it))]
    }
    if (class(try(save.GR2[count,] <- save.it[save.it[,7] == min(save.it[,7]), 
                                              c(1:3,7,12:ncol(save.it))],silent = T)) != "try-error") {
      save.GR2[count,] <- save.it[save.it[,7] == min(save.it[,7]), 
                                  c(1:3,7,12:ncol(save.it))]
    }
    if (class(try(save.GR3[count,] <- save.it[save.it[,8] == min(save.it[,8]), 
                                              c(1:3,8,12:ncol(save.it))],silent = T)) != "try-error") {
      save.GR3[count,] <- save.it[save.it[,8] == min(save.it[,8]), 
                                  c(1:3,8,12:ncol(save.it))]
    }
    if (class(try(save.GR4[count,] <- save.it[save.it[,9] == min(save.it[,9]), 
                                              c(1:3,9,12:ncol(save.it))],silent = T)) != "try-error") {
      save.GR4[count,] <- save.it[save.it[,9] == min(save.it[,9]), 
                                  c(1:3,9,12:ncol(save.it))]
    }
    if (class(try( save.GR5[count,] <- save.it[save.it[,10] == min(save.it[,10]), 
                                               c(1:3,10,12:ncol(save.it))],silent = T)) != "try-error") {
      save.GR5[count,] <- save.it[save.it[,10] == min(save.it[,10]), 
                                  c(1:3,10,12:ncol(save.it))]
    }
    if (class(try(save.GR6[count,] <- save.it[save.it[,11] == min(save.it[,11]), 
                                              c(1:3,11,12:ncol(save.it))],silent = T)) != "try-error") {
      save.GR6[count,] <- save.it[save.it[,11] == min(save.it[,11]), 
  ...
  colnames(top.configs) <- c("p", "I", "ET", "value", 
                             "PSe", "PSp", "PPPV", "PNPV", 
